...
int food(int *flag, user* users, order* ord)
{
    ...
    char selected_address[30] = "请选择您的收货地址"; // 默认地址提示
    ShoppingCart cart; 
    CartNode* curr = NULL;
    ...
    while (1)
    {
        ...
        else if(current_screen == 7)
        {
            ...
            {
                // 画支付页面
                draw_payment(&cart, users, selected_address);
                payment_drawn = 1; 
            }
                ...
                if(check_balance(users->balance, cart.total_price))
                {
                    if(update_user_balance(users, cart.total_price) == 0)
                    {
                        if(update_inventory(&cart) == 0)
                        {
                            ...
                            init_order(&cart, users, current_store->id, delivery_address, CATEGORY_FOOD, ord);          
                            cart_clear(&cart); // 清空购物车
                            current_screen = 0; 
        ...
        else if(current_screen == 8)
        {
            ...
            if(delivery_address != 0)
            {
                ...
                {
                    case 101: strcpy(selected_address, "韵苑25栋学生宿舍"); break;
                    case 102: strcpy(selected_address, "东九教学楼"); break;
                    case 103: strcpy(selected_address, "东十二教学楼"); break;
                    case 104: strcpy(selected_address, "韵苑26栋学生宿舍"); break;
                    case 105: strcpy(selected_address, "韵苑19栋学生宿舍"); break;
                    case 106: strcpy(selected_address, "韵苑5栋学生宿舍"); break;
                    case 107: strcpy(selected_address, "韵苑10栋学生宿舍"); break;
                }
                current_screen = 7;
                address_drawn = 0;
            }
...
void draw_payment(ShoppingCart* cart, user* u, const char* selected_address)
{
    ...
    puthz(x + 140, y + 10, "支付页面", 32, 32, 0x001f);
    ...
    // puthz(400, 525, "请选择您的收获地址", 24, 24, 0x8410);
    prt_hz16_asc16_size(400, 525, 1.5, 1.5, selected_address, 0x8410, "HZK\\HZK24k"); // 显示选择的地址
    // 确认支付按钮
...
/*
功能：将购物车内容保存到订单结构体
input：ShoppingCart* cart, user* u, unsigned short payment_addr, unsigned short delivery_addr, StoreCategory type
output：0表示成功，-1表示失败
*/
int init_order(const ShoppingCart* cart, const user* u, unsigned short payment_addr, unsigned short delivery_addr, StoreCategory type, order* ord)
{
    order new_order;
    CartNode* curr = cart->head;
    OrderItem* item = NULL;
    ...
    {
        CloseSVGA();
        printf("Invalid parameters in save_order\n");
        exit(1);
        return -1;
    }

    // 填充订单基本信息
    strncpy(new_order.name, u->name, sizeof(new_order.name) - 1);
    new_order.name[sizeof(new_order.name) - 1] = '\0'; 
    new_order.type = type;
    new_order.state = SearchDeliver;
    generate_order_num(new_order.order_num, sizeof(new_order.order_num));
    new_order.money = cart->total_price;
    new_order.time = time(NULL);
    if (payment_addr == 1 || payment_addr == 2) payment_addr = 208;
    else if (payment_addr == 3 || payment_addr == 4) payment_addr = 203; 
    new_order.payment_address = payment_addr; // 支付地址
    new_order.delivery_address = delivery_addr; // 收货地址
    new_order.item_count = 0; // 初始化商品数量为 0

    // 填充订单商品项
    while (curr) 
    {
        if (new_order.item_count >= MAX_ORDER_ITEMS) {
            printf("Order item limit reached.\n");
            break;
        }

        item = &new_order.items[new_order.item_count];
        strncpy(item->name, curr->item->name, sizeof(item->name) - 1);
        item->name[sizeof(item->name) - 1] = '\0';
        item->store_id = curr->item->store_id;
        item->price = curr->item->price;
        item->quantity = curr->quantity;

        new_order.item_count++; // 增加商品数量
        curr = curr->next;
    }
...
void generate_order_num(char* buffer, size_t size) {
    ...
    if (size > 0) {
        sprintf(buffer, "ORD%04d%02d%02d%02d%02d%02d",
...
//     printf("Payment Address: %d\n", ord->payment_address);
//     printf("Delivery Address: %d\n", ord->delivery_address);

//     printf("Order Time: %s", ctime(&ord->time));
//     printf("Item Count: %d\n", ord->item_count);

//     // 打印订单商品列表
//     printf("Order Items:\n");
//     for (i = 0; i < ord->item_count; i++) 
//     {
//         OrderItem* item = &ord->items[i];
//         printf("  Item %d:\n", i + 1);
//         printf("    Store ID: %d\n", item->store_id);
//         printf("    Price: %.2f\n", item->price);
//         printf("    Quantity: %d\n", item->quantity);
